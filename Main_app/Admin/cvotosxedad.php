...
<head>
 ...
 ['Browser', 'Visits'],
 <?php 
 $query = "SELECT DISTINCT edad, COUNT(edad) as total FROM votante WHERE estado = 'Correcto' AND id_candidato = '$id' GROUP BY edad";

 $exec = mysqli_query($con,$query);
 while($row = mysqli_fetch_array($exec)){

 echo "['".$row['edad']."',".$row['total']."],";
 }
 ?>
 ]);

 var options = {
 title: 'Estadísticas por Edad'
 };
